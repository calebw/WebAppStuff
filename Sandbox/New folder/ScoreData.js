...
var ScoreData={
	...
	getRangeData: function(dateFrom, dateTo, callback){
		open();
		//console.log("SELECT KeyName, FileName, Score, RunDate FROM scores where RunDate > '"+dateFrom+"' AND RunDate <= '"+dateTo+"'");
		con.query("SELECT KeyName, FileName, Score, RunDate FROM scores where RunDate > ? AND RunDate < ?", [dateFrom, dateTo], function(err, rows){
			if(err) {
				//console.log(err);
				close();
				callback(err, []);
